...
class BehavioralCloningTrainer(OAITrainer):
    def __init__(self, dataset, args, name=None, layout_names=None, vis_eval=False):
        ...
        p_encoding_fn = ENCODING_SCHEMES['OAI_feats']
        self.eval_envs = [OvercookedGymEnv(shape_rewards=False, is_eval_env=True, p_encoding_fn=p_encoding_fn, horizon=400,
                                           layout_name=ln, args=args) for ln in self.layout_names]
        obs = self.eval_envs[0].get_obs(p_idx=0, enc_fn=p_encoding_fn)
